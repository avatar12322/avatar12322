# Cześć, jestem Szymon!

### Student Informatyki | Analityk Danych

Obecnie studiuję Informatykę na **WSEI w Krakowie**. Fascynuje mnie sztuczna inteligencja, uczenie maszynowe oraz analiza danych, ze szczególnym uwzględnieniem praktycznych zastosowań modeli językowych i analitycznych.

**[>> Kliknij tutaj, aby zobaczyć moje pełne CV <<](https://flowcv.com/resume/osqdw66b9rr6)**

### O mnie
* Student Informatyki ze specjalizacją Programowanie i analiza danych.
* Twórca **Wirtualnego Instruktora Lotu** – aplikacji weryfikującej procedury w oparciu o modele uczenia maszynowego (ONNX) wytrenowane na rzeczywistych danych telemetrycznych z NASA.
* Skupiam się na krytycznym myśleniu, logice oraz testowaniu modeli AI pod kątem spójności, realizmu i wartości edukacyjnej.

### Technologie i Narzędzia
![C#](https://img.shields.io/badge/c%23-%23239120.svg?style=for-the-badge&logo=csharp&logoColor=white)
![.Net](https://img.shields.io/badge/.NET-5C2D91?style=for-the-badge&logo=.net&logoColor=white)
![TypeScript](https://img.shields.io/badge/typescript-%23007ACC.svg?style=for-the-badge&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/react-%2320232a.svg?style=for-the-badge&logo=react&logoColor=%2361DAFB)
![SQL](https://img.shields.io/badge/sql-%23003B57.svg?style=for-the-badge&logo=mysql&logoColor=white)

---

### Wyróżnione Projekty

**Wirtualny Instruktor Lotu (Analiza Danych / AI)**

[![Wirtualny Instruktor Lotu](https://github-readme-stats.vercel.app/api/pin/?username=avatar12322&repo=FlightDataRecorder&theme=radical)](https://github.com/avatar12322/FlightDataRecorder)

**Questify (Grywalizacja i Nawyki)**

[![Questify](https://github-readme-stats.vercel.app/api/pin/?username=avatar12322&repo=questify&theme=radical)](https://github.com/avatar12322/questify)
*Aplikacja w stylu RPG, która zamienia codzienne obowiązki w angażujące wyzwania. Wykonywanie realnych zadań (np. sprzątanie) pozwala rozwijać postać, zdobywać doświadczenie i wirtualną walutę.*

---

### Moje Statystyki na GitHubie
![Statystyki Szymona](https://github-readme-stats.vercel.app/api?username=avatar12322&show_icons=true&theme=radical)
